...
def richer_than_england
  ...
  # 'United Kingdom'.
  # per capita gdp of United Kingdom
  execute(<<-SQL)
  SELECT
    name
  FROM
...
def neighbors_of_certain_b_countries
  ...
  # 'Belize', 'Belgium'.
  # continents containing belize and belbgium
  # name and continent of countries in those continents
  execute(<<-SQL)
    SELECT
      name, continent
    FROM
...
def population_constraint
  ...
  FROM
    countries
  WHERE
    name = 'Canada') AND population < (SELECT
      population 
    FROM
      countries
    WHERE
...
def sparse_continents
  ...
  #country's name, con, pop that belongs to the contient(all countries pop < 25mil)
  # find country
  #continent with countries pop less than 25 mil
  execute(<<-SQL)
  SELECT
    continent
  FROM
